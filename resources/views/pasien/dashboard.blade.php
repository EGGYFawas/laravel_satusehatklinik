...
<<<<<<< Updated upstream
        <div class="w-full max-w-lg bg-white rounded-xl shadow-lg p-6 text-center mb-8"><img src="{{ asset('assets/img/ambil_antrean.png') }}" alt="Antrean Online" class="w-32 h-32 mx-auto mb-4"><h3 class="text-xl font-bold text-gray-800">Antrean Online</h3><p class="text-gray-500 mb-6">Daftar antrean berobat menjadi lebih mudah.</p><button id="ambilAntrianBtn" class="bg-[#24306E] hover:bg-[#1a224d] text-white font-bold py-3 px-8 rounded-lg transition duration-300 shadow-md">Ambil Antrian</button></div>
        <div class="w-full max-w-5xl grid grid-cols-1 md:grid-cols-2 gap-8"><div class="bg-white rounded-xl shadow-lg p-6"><h3 class="text-lg font-bold text-gray-800 border-b pb-2 mb-4">Nomor Antrean Berobat</h3>@if($antreanBerobat)<div class="text-center py-4"><p class="text-sm text-gray-500 mb-2">Poli {{ $antreanBerobat->poli->name }}</p><p class="text-6xl font-extrabold text-[#24306E]">{{ $antreanBerobat->queue_number }}</p><p class="text-lg text-yellow-600 font-semibold mt-4 bg-yellow-100 rounded-full px-4 py-1 inline-block">Status: {{ ucwords(strtolower($antreanBerobat->status)) }}</p></div>@else<div class="text-center text-gray-500 py-8"><p>Belum ada antrean dibuat hari ini.</p></div>@endif</div><div class="bg-white rounded-xl shadow-lg p-6"><h3 class="text-lg font-bold text-gray-800 border-b pb-2 mb-4">Nomor Antrean Apotik</h3><div class="text-center text-gray-500 py-8"><p>Belum ada resep obat terbaru.</p></div></div></div>
=======
        ...
        <div class="w-full max-w-5xl grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                @if($antreanBerobat)
                    ...
                    <div class="mt-6 space-y-4">
                        ...
                        <div class="flex justify-between items-center bg-gray-50 p-3 rounded-lg">
                            ...
                            @php
                                $estimasi = '-';
                                if ($antreanBerjalan && in_array($antreanBerobat->status, ['MENUNGGU', 'HADIR'])) {
                                    ...
                                    $waktuTunggu = ($selisih < 0 ? 0 : $selisih) * 15;
                                    $estimasi = $waktuTunggu > 0 ? "sekitar {$waktuTunggu} menit lagi" : "Segera";
                        ...
                        @if($antreanBerobat->status == 'MENUNGGU')
                            <div class="pt-4 border-t">
                                <button id="checkInBtn" class="w-full bg-green-500 hover:bg-green-600 text-white font-bold py-3 px-6 rounded-lg transition duration-300 shadow-md text-base">
                                    <svg class="w-6 h-6 inline-block mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 9a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2V9z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                                    Saya Sudah Tiba, Lakukan Check-In
                                </button>
                            </div>
                        @elseif(in_array($antreanBerobat->status, ['HADIR', 'DIPANGGIL', 'SELESAI']))
                            <div class="pt-4 border-t">
            ...
            <div class="bg-white rounded-xl shadow-lg p-6">
                ...
                @if($antreanApotek)
                    @php
                        $statusApotekText = ''; $bgApotekColor = ''; $textApotekColor = '';
                        switch ($antreanApotek->status) {
                            case 'MENUNGGU_RACIK':
                                $statusApotekText = 'Menunggu Obat Disiapkan'; $bgApotekColor = 'bg-red-100'; $textApotekColor = 'text-red-800'; break;
                            case 'SEDANG_DIRACIK':
                                $statusApotekText = 'Obat Sedang Disiapkan'; $bgApotekColor = 'bg-yellow-100'; $textApotekColor = 'text-yellow-800'; break;
                            case 'SIAP_DIAMBIL':
                                $statusApotekText = 'Obat Siap Diambil'; $bgApotekColor = 'bg-green-100'; $textApotekColor = 'text-green-800'; break;
                            case 'SELESAI':
                                $statusApotekText = 'Obat Sudah Diterima'; $bgApotekColor = 'bg-gray-100'; $textApotekColor = 'text-gray-800'; break;
                        }
                    @endphp
                     <div class="text-center py-4">
                        <p class="text-sm text-gray-500 mb-2">Status Resep Anda:</p>
                        <p class="text-6xl font-extrabold text-[#24306E]">{{ $antreanApotek->pharmacy_queue_number }}</p>
                        <p class="text-lg {{ $textApotekColor }} {{ $bgApotekColor }} font-semibold mt-4 rounded-full px-4 py-1 inline-block">{{ $statusApotekText }}</p>
                    </div>
                @else
...
>>>>>>> Stashed changes
    ...
    <div class="mt-12 w-full max-w-5xl mx-auto"><h2 class="text-2xl font-bold text-gray-800 mb-6">Artikel Kesehatan Terbaru</h2>@if(isset($articles) && $articles->isNotEmpty())<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">@foreach($articles as $article)<div class="bg-white rounded-xl shadow-lg overflow-hidden flex flex-col transform hover:-translate-y-2 transition-transform duration-300"><img src="https://placehold.co/600x400/ABDCD6/24306E?text=Klinik+Sehat" alt="Gambar Artikel: {{ $article->title }}" class="w-full h-48 object-cover"><div class="p-6 flex-grow flex flex-col"><h3 class="font-bold text-lg mb-2 text-gray-800">{{ $article->title }}</h3><p class="text-gray-600 text-sm flex-grow">{{ Str::limit(strip_tags($article->content), 100) }}</p><a href="#" class="text-sm text-[#24306E] font-semibold mt-4 self-start hover:underline">Baca Selengkapnya &rarr;</a></div></div>@endforeach</div>@else<div class="text-center text-gray-500 py-8 bg-white rounded-xl shadow-lg"><p>Belum ada artikel kesehatan yang diterbitkan.</p></div>@endif</div>
@endsection
            ...
            <div class="overflow-y-auto p-8 flex-grow">
                ...
                <form id="antrianForm" action="{{ route('pasien.antrean.store') }}" method="POST">
                    ...
                    <div class="border-t border-gray-200 pt-6">
                        {{-- MODIFIKASI: Blok ini hanya tampil jika mendaftar untuk diri sendiri --}}
                        ...
                        <!-- Data Pasien yang Didaftarkan (Kondisional) -->
                        ...
                        <!-- Data Pendaftaran (Selalu Tampil) -->
...
=======
    @if(!$antreanBerobat)
        <div id="antrianModal" class="hidden fixed inset-0 bg-gray-900 bg-opacity-70 flex items-center justify-center z-50 p-4">
             <div id="modalContent" class="bg-white rounded-xl shadow-2xl w-full max-w-4xl flex flex-col max-h-[95vh] transform transition-all" x-data="{ isFamily: false, customRelationship: false, nikInput: '' }">
                 <div class="text-center p-6 border-b border-gray-200 flex-shrink-0">
                     <h3 class="text-2xl font-bold text-gray-800">Formulir Antrean Baru</h3>
                 </div>
                 <div class="overflow-y-auto p-8 flex-grow">
                         ...
                         <div class="border-t border-gray-200 pt-6">
                             ...
                             <div x-show="isFamily" x-transition class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 mb-4 border-b border-gray-200 pb-4">
                                 ...
                                 <div class="md:col-span-2">
                                      ...
                                      <select name="patient_relationship" @change="customRelationship = ($event.target.value === 'Lainnya')" class="w-full p-2 border border-gray-300 rounded-md" :required="isFamily">
                                         <option value="" disabled selected>-- Pilih Hubungan --</option>
                                         <option value="Anak">Anak</option> <option value="Orang Tua">Orang Tua</option>
                                         <option value="Pasangan">Pasangan</option> <option value="Saudara Kandung">Saudara Kandung</option>
                                         <option value="Lainnya">Lainnya</option>
                                      </select>
                                 </div>
                                 <div x-show="customRelationship" x-transition class="md:col-span-2">
                             ...
                             <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                                  <div x-show="!isFamily" class="md:col-span-2"><h4 class="text-lg font-semibold text-gray-700 mb-2 border-t border-gray-200 pt-4">Detail Pendaftaran</h4></div>
                                  <div x-show="isFamily" class="md:col-span-2"><h4 class="text-lg font-semibold text-gray-700 mb-2">Detail Pendaftaran</h4></div>
                                  <div>
                                     <label for="poli" class="block text-sm font-medium text-gray-700 mb-1">Pilih Poli</label>
                                     <select id="poli" name="poli_id" class="w-full p-2 border border-gray-300 rounded-md" required>
                                         <option value="" disabled selected>-- Silahkan Pilih Poli --</option>
                                         @foreach($polis as $poli) <option value="{{ $poli->id }}">{{ $poli->name }}</option> @endforeach
                                     </select>
                                 </div>
                                 <div>
                                     <label for="doctor" class="block text-sm font-medium text-gray-700 mb-1">Pilih Dokter</label>
                     ...
                     @else
                         <div class="text-center p-8"><p class="text-red-600 font-semibold">Data profil pasien tidak ditemukan.</p><p class="text-gray-600 mt-2">Harap lengkapi profil Anda terlebih dahulu.</p></div>
                     @endif
                 </div>
                 <div class="flex justify-center items-center gap-4 p-6 border-t border-gray-200 flex-shrink-0">
                     <button type="button" id="cancelModalBtn" class="bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold py-2 px-6 rounded-lg">Batal</button>
...
@push('scripts')
<<<<<<< Updated upstream
{{-- Script tidak diubah, karena logika Alpine.js sudah menangani tampilan --}}
<script>
document.addEventListener('DOMContentLoaded', function () {
    try {
        ...
        function closeModal() { 
            ...
            if (antrianForm && modalContent && modalContent.__x) {
                modalContent.__x.data.isFamily = false;
                modalContent.__x.data.customRelationship = false;
                modalContent.__x.data.nikInput = '';
                antrianForm.reset();
            }
        }

        if (ambilAntrianBtn) { ambilAntrianBtn.addEventListener('click', openModal); }
        ...
        if (antrianForm) {
            antrianForm.addEventListener('submit', function(e) {
                ...
                Swal.fire({
                    ...
                    text: "Pastikan semua data yang Anda masukkan sudah benar sebelum disimpan.",
                    icon: 'question',
                    showCancelButton: true,
        ...
        if (checkInBtn) {
            ...
            checkInBtn.addEventListener('click', () => {
                ...
                html5QrCode.start({ facingMode: "environment" }, config, qrCodeSuccessCallback)
                    .catch(err => {
                        Swal.fire({ icon: 'error', title: 'Gagal Membuka Kamera', text: 'Pastikan Anda memberikan izin akses kamera pada browser.' });
...
>>>>>>> Stashed changes
        ...
        console.error("Terjadi kesalahan pada skrip dashboard:", error);
        Swal.fire({
            icon: 'error',
            title: 'Oops... Terjadi Kesalahan',
            text: 'Ada masalah pada skrip halaman. Silakan muat ulang halaman atau hubungi administrator.',
            footer: '<b class="text-red-500">Info untuk Developer: Periksa console untuk detail error.</b>'
        });
    }
});
</script>
