...
<div class="w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    {{-- Form Pencarian --}}
    <div class="mb-8">
        <form action="{{ route('artikel.index') }}" method="GET"> {{-- [MODIFIKASI] Route publik --}}
            <div class="relative">
                <input
                    type="text"
                    name="search"
    ...
    @if($articles->count() > 0)
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            @foreach($articles as $article)
                <a href="{{ route('artikel.show', $article->slug) }}" class="article-card bg-white rounded-xl shadow-lg overflow-hidden flex flex-col no-underline hover:no-underline border">
    ...
    @else
        <div class="text-center text-gray-500 py-16 bg-white rounded-xl shadow-lg border">
